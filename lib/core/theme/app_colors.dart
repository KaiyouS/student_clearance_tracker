...
abstract final class AppColors {
  ...
  // Transitional adapter for widgets still using AppColors.of(context).
  static AppColorAliases of(BuildContext context) {
    return AppColorAliases.fromTheme(Theme.of(context));
  }

  static Color statusColorFromString(BuildContext context, String status) {
    return of(context).statusFromString(status);
  }
}

@immutable
final class AppColorAliases {
  const AppColorAliases({
    required this.info,
  ...
  factory AppColorAliases.fromTheme(ThemeData theme) {
    final isDark = theme.brightness == Brightness.dark;
    final scheme = theme.colorScheme;
    return AppColorAliases(
      info: scheme.primary,
      success: scheme.tertiary,
      warning: AppColors.warning,
      danger: scheme.error,
      neutral: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary,
      statusPending: AppColors.warning,
      statusSigned: scheme.tertiary,
      statusFlagged: scheme.error,
      statusInactive: isDark ? AppColors.darkBorder : AppColors.lightBorder,
      border: theme.dividerColor,
    );
  }

  Color statusFromString(String status) => switch (status.toLowerCase().trim()) {
    'signed' => statusSigned,
    'flagged' => statusFlagged,
    'inactive' || 'disabled' => statusInactive,
    _ => statusPending,
  };
}
